...
const DesktopDropdown = ({
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((p) => !p)}
        className={`flex items-center gap-1.5 text-sm font-medium transition-colors duration-300 ${isScrolled
...
const Header = ({ transparentAtTop = false }: HeaderProps) => {
  ...
  return (
    <>
      {/* HEADER */}
      <header
        className={`fixed top-0 left-0 right-0 z-[9999] transition-all duration-500 ${isScrolled ? "py-2 px-4" : "py-4 px-4"
          }`}
      >
        <motion.div
        ...
        >
          ...
          <div className="flex items-center gap-8 lg:gap-10">
            ...
            <Link to="/" className="flex items-center gap-3 shrink-0">
              <img
                src={logo}
                alt="Triponomic"
                className={`h-8 md:h-9 w-auto transition-all duration-300 ${isScrolled ? "" : "brightness-0 invert"
                  }`}
              />
              <span
                className={`font-semibold text-lg transition-colors duration-300 ${isScrolled ? "text-foreground" : "text-white"
                  }`}
              >
                Triponomic
              </span>
          ...
          <div className="hidden md:flex items-center gap-3">
            <a
              ...
              className={`flex items-center gap-2 px-5 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition-all duration-300 ${isScrolled
            ...
            <a
              ...
              className={`flex items-center justify-center p-2 rounded-full transition-colors duration-300 ${
                isScrolled
                  ...
                  : "bg-emerald-600/90 hover:bg-emerald-700 border border-emerald-500/40 text-white shadow-[0_2px_12px_rgba(5,150,105,0.35)]"
              }`}
              aria-label="Call Us"
            >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[10000] bg-white text-gray-900 flex flex-col justify-between overflow-y-auto"
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 bg-white">
              ...
              >
                ...
                />
                <span className="font-semibold text-lg text-gray-900">Triponomic</span>
              </Link>

              <div className="flex items-center gap-2">
                <a
                  href="tel:+919611922632"
                  className="w-9 h-9 rounded-full bg-emerald-50 border border-emerald-200 flex items-center justify-center text-emerald-600 hover:bg-emerald-100 transition active:scale-95"
                  aria-label="Call Us"
                >
                  <Phone className="w-4 h-4" />
                </a>
                ...
                <button
                  type="button"
                  onClick={() => setMobileMenuOpen(false)}
                  className="w-9 h-9 rounded-full bg-gray-100 border border-gray-200 flex items-center justify-center text-gray-700 hover:bg-gray-200 transition active:scale-95"
            ...
            <div className="px-5 py-5 space-y-2 flex-1">
              {[
                ...
                },
                {
                  ...
                  sub: "Best 5-star hotels at the best prices for you",
                  icon: Hotel,
                },
                {
                  to: "/experiences",
                  label: "Curated Experiences",
                  sub: "Fun activities, dining & unique local adventures",
                  icon: Compass,
                },
                {
                  to: "/build-trail",
                  label: "Build Your Journey",
                  sub: "Create your perfect trip, your way",
                  icon: Sparkles,
                  highlight: true,
                },
                {
                  ...
                  sub: "Who we are and how we make travel special",
                  icon: Globe,
                },
                {
                  to: "/faq",
                  ...
                  >
                    <div className="flex items-center gap-3.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <span className={`text-sm font-bold truncate ${item.highlight ? "text-white" : "text-gray-900"}`}>
                            {item.label}
                          </span>
                          {item.tag && (
                            <span
                        ...
                        <p className={`text-[11px] truncate mt-0.5 font-light ${item.highlight ? "text-white/70" : "text-gray-500"}`}>
                          {item.sub}
                        </p>
                      </div>
                    </div>

                    <ArrowRight className={`w-4 h-4 shrink-0 ml-2 ${item.highlight ? "text-white/40" : "text-gray-400"}`} />
                  </Link>
                );
              })}
            </div>
            ...
            <div className="p-5 border-t border-gray-200 bg-gray-50 space-y-3">
              <div className="grid grid-cols-2 gap-2.5">
                <Link
                  to="/enquire"
                  onClick={() => setMobileMenuOpen(false)}
                  className="flex items-center justify-center gap-2 py-3 px-3 rounded-xl bg-white hover:bg-gray-100 active:scale-95 text-gray-800 text-xs font-bold transition border border-gray-200"
                >
